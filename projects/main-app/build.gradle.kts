...
tasks.withType<ShadowJar> {
    ...
    // 不要重定位net.kyori包，因为Velocity API依赖原始的net.kyori包
    // relocate("net.kyori", "com.beanbeanjuice.simpleproxychat.libs.net.kyori")
    
    // 明确指定输出目录和文件名
    destinationDirectory.set(file("${project.buildDir}/output"))
    archiveFileName.set("${project.name}-${project.version}-all.jar")
}

// 添加一个任务来打印 ShadowJar 的输出路径
tasks.register("printShadowJarPath") {
    doLast {
        val shadowJar = tasks.named<ShadowJar>("shadowJar").get()
        println("ShadowJar 输出路径: ${shadowJar.archiveFile.get().asFile.absolutePath}")
    }
}